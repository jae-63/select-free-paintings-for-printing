    ...
    for rec in records:
        ...
        if check_resolution and (px_w == 0 or px_h == 0):
            ...
            if image_id and source == "aic":
                # AIC uses its own IIIF base URL
...
def parse_args():
    ...
    p.add_argument("--sources", nargs="+",
                   ...
                   choices=["met", "aic", "europeana", "rijksmuseum", "getty"],
...
def main():
    ...
    if "europeana" in args.sources:
        from sources.europeana import fetch_all_candidates, WATERCOLOR_QUERIES, OIL_QUERIES
        # Always fetch watercolors and oils as SEPARATE passes, each with the
        # full limit. This prevents the watercolor target consuming the entire
        # budget before oil queries are reached.
        if args.watercolor_target > 0:
            print("[Europeana] Fetching watercolor candidates...")
            all_records += fetch_all_candidates(limit=args.limit, queries=WATERCOLOR_QUERIES)
    ...
    if "rijksmuseum" in args.sources:
        ...
        if args.watercolor_target > 0:
            ...
            all_records += rij_fetch(queries=RIJ_WC_Q, limit=args.limit)
        if args.oil_target > 0:
            print("[Rijksmuseum] Fetching oil candidates...")
            all_records += rij_fetch(queries=RIJ_OIL_Q, limit=args.limit)
    ...
    if len(watercolors) < args.watercolor_target:
        ...
        print(f"   --limit {args.limit * 3}   (fetch more candidates per source)")
